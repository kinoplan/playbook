...
* **Пакеты** - строчными буквам ASCII и одним словом (если возникает необходимость в нескольких словах - стоит подумать о вложенных пакетах).
...
* **Перечисления** должны быть в **UpperCamelCase**.
  ...
  ```scala
    object Status extends Enumeration {
      ...
      val Installing = Value("installing")
    }
  ```
...
* Используйте круглые скобки для методов без параметров, которые выполняет какие-то изменения (например, сеттеры).
	...
	```
* Во всех остальных случаях для методов без параметров скобки опускаются.
...
Ставьте фигурные скобки даже вокруг однострочных условных или петлевых операторов. Исключение - использование `if/else` как однострочный тернарный опеартор.
...
* Если функция принимает 1 аргумент, то аргументы и подчеркивания должны быть опущены:
...
Вывод типа Scala, особенно левосторонний вывод типа и вывод закрытия в большинстве случаев делает код более кратким. Однако, есть несколько случаев, когда следует использовать явный тип:

* **Методы в DAO должны быть явно типизированы.** Это вносит ясность в то, что метод достает из базы, а также уменьшает риск сделать в дальнейшем ошибку.
* **Неявные методы должны быть явно типизированы,** иначе это может привести к сбою Scala компилятора с инкрементной компиляцией.
...
**Используйте рекурсию разумно.** Вы можете сипользовать ее для решения стандарнтых задач (например, обход дерева, обход графа и т.д.), а также для задач которые проще решить через рекурсию, если заведомо известно, что не будет большого погружения. Однако, если то же самое можно сделать с помощью стандартных циклов и это будет выглядеть проще, то отдавайте предпочтение им.

Для методов, предназначенных для рекурсивной обработки, старйтесь по возможности применять аннотацию `@tailrec`, если не происходят асихронные операции в рекусрии. Во-первых, тем самым компилятор Scala проверяет метод на рекурсивность(например, что не используются замыкания, функциональные преобразования и т.д.). Во-вторых, компилятор оптимизирует рекурсию. 

### <a name='implicits'>Неявные преобразования (implicits)</a>
**Избегайте использования неявных преобразований,** за исключением:
...
// хорошо
def get(releaseId: Int): Option[Release] = ...
```
...
* Если же при построении `Option` используется преобразование, которое может вернуть `null`, то используйте `Option`.
	...
	```
* Используйте `None` для представления исключения, вместо явной выдачи исключения.
* Для раскрытия `Option` рекомендуется использовать связку `map` и `getOrElse`, вместо `fold`. Для раскрытия `Option` разрешается использование `fold` только в особо лаконичных случаях.
	
	```scala
	// хорошо в особо лаконичных случаях
	Option(123).fold(0)(...)
	
	// хорошо
	Option(123).map(...).getOrElse(0)
	
	// плохо
	Option(123).fold(0)(
	//какой-то здоровый кусок кода
	...
	
	)
	```
* Для раскрытия `Option` разрешается использовать `match` по усмотрению разработчика.
...
* **conf/** - конфиги
	* **application.conf** - конфигурационные файлы, может быть несколько
	* **logback.xml** - правила логирования
...
* `Writes` разрешается создавать как в нотации функции, так и в стандартной натации. Однако старайтесь в рамках одного проекта придерживаться конкретной нотации.
 
	```scala
	// хорошо
	val writes: Writes[Agency] = (o: Agency) => { ... }
	
	// хорошо
	def writes = new Writes[Agency] {
      def writes(o: Agency): JsValue = {
        Json.obj(
          ...
        )
      }
